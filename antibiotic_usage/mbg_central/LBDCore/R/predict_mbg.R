...
predict_mbg <- function(res_fit,
  ...
  if (coefs.sum1) {
    ...
    l_idx_int <- match(
      sprintf("%s.1", res_fit$names.fixed), ## main effects
      par_names
    )
  ...
  if (coefs.sum1) {
    rownames(pred_l) <- c(
      stacker_names, ## covars in sum-to-1
      res_fit$names.fixed
    ) ## intercept
  } else {
  ...
  A.pred.s.no.t <- inla.spde.make.A(
    mesh = mesh_s,
    loc = gp_coords
  )
