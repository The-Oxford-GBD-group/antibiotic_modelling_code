...
#' @export
#'
estimate_parallel_model_mem <- function(regions = NULL, npixels = NULL, nyears, ndraws, core_repo = "/share/code/geospatial/lbd_core/", shapefile_version = "current", print.tables = FALSE) {
  ...
  if (length(nyears) != 1 | length(ndraws) != 1) {
    warning("This function was only tested using integer values for ndraws and nyears.")
    warning("You appear to have entered in vectors for at least one of these arguments and we do not promise that this function will work as expected with your inputs!")
  }
  ...
  train.dat <- data.table(matrix(0, ncol = 3, nrow = nreg))
  ...
  if (is.null(regions)) {
    ...
    for (ii in 1:nreg) {
      ...
      if (length(adm0_list) == 0) {
        ...
        polygon_list <- suppressMessages(load_simple_polygon(gaul_list = adm0_list, buffer = 1, tolerance = 0.4, shapefile_version = shapefile_version))
  ...
  ## load in the ACE/AVAS model fits that have been estimated load(paste0(core_repo, '/mbg_central/util/slot_estimation_fits.RData'))
  data(ace.r) ## Result of model fit object
  data(fss) ## function to perform regression given model in ace.r
  data(percent.over)
  data(res.sd)
  ...
  train.dat[, `:=`(ram.pred0sd, exp(fss$log_ram_gb(predict(ace.r, train.dat) + 0 * res.sd)))] ## no sd buffer
  train.dat[, `:=`(ram.pred1sd, exp(fss$log_ram_gb(predict(ace.r, train.dat) + 1 * res.sd)))] ## 1 sd buffer
  train.dat[, `:=`(ram.pred2sd, exp(fss$log_ram_gb(predict(ace.r, train.dat) + 2 * res.sd)))] ## 2 sd buffer
  train.dat[, `:=`(ram.pred3sd, exp(fss$log_ram_gb(predict(ace.r, train.dat) + 3 * res.sd)))] ## 3 sd buffer

  ## now we print out the results

  ram.list <- NULL
  for (rr in 1:nreg) {
    dt <- data.table(RAM = t(train.dat[rr, .(ram.pred0sd, ram.pred1sd, ram.pred2sd, ram.pred3sd)]) / 1000, EXCEEDENCE = percent.over)
