...
#'
#' @param addl_job_name Additional name appended to job name
#'
...
#'
#'  @param use_argparse Add argparse flags for parallel model arguments? Default: FALSE
#'
...
make_qsub_share <- function(user = Sys.info()["user"],
  ...
  # A sub-function to append argparse flag if we're passing it
  ...
  qsub <- generate_qsub_command(
    ...
    shell,
    code,
    argparse_append(reg, "reg", use_argparse),
    argparse_append(age, "age", use_argparse),
    argparse_append(rd, "run_date", use_argparse),
    argparse_append(as.numeric(test), "test", use_argparse),
    argparse_append(holdout, "holdout", use_argparse),
    argparse_append(indic, "indic", use_argparse),
    argparse_append(ig, "ig", use_argparse),
    "fin"
  )
